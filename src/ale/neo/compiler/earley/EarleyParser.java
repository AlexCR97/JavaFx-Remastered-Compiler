...
public class EarleyParser {
    ...
    public boolean parse(List<String> tokens) {
        sentence = tokens;
        charts = new Chart[sentence.size() + 1];
        ...
        State endState = new State("$", endRHS, 0, sentence.size());

        for (int j = 0; j < charts[sentence.size()].size(); j++) {
            State state = charts[sentence.size()].getState(j);
        ...
        for (int a = 0; a < rhs.length; a++) {
            ...
            if (j < sentence.size()) {
                for (int k = 0; k < count; k++) {
                    ...
                    String sent = sentence.get(j).toLowerCase();
